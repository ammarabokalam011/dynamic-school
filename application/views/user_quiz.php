                            ...
                            if($user_quiz!=null) {
                                foreach ($user_quiz as $row) {
                                    ...
                                    if($row['start_date_time']> date('Y-m-d H:i:s') && $row['end_date_time']> date('Y-m-d H:i:s')){
                                        ...
                                        echo '<td> <span class="m-badge m-badge--brand m-badge--wide">في انتظار بدء المسابقة بعد  '.$final_hour.'ساعة و '.$final_minut.'دقيقة'.'</span></td>';
                                    }elseif($row['start_date_time']< date('Y-m-d H:i:s') && $row['end_date_time']< date('Y-m-d H:i:s')){
                                        echo '<td> <span class="m-badge m-badge--brand m-badge--danger">المسابقة منتهية</span></td>';
                                    }elseif($row['start_date_time']< date('Y-m-d H:i:s') && $row['end_date_time']> date('Y-m-d H:i:s')){
                                        echo '<td> '.
                                            '<form action="'.base_url('user_quiz/login_quiz').'" method="post">'.
                                                 '<input type="hidden" name="id_quiz" value="'.$row['id'].'">'.
                                                 '<input type="hidden" name="id_user" value="'.$user_id.'">'.
                                                 '<button type="submit" class="m-badge m-badge--brand m-badge--accent">دخول المسابقة</button></td>'.
                                            '</form>';
                                    }
...
<?php
                        ...
                        foreach ($all_quiz as $row) {
                            ...
                            '<td><form action="'.base_url('user_quiz/Participate_competition').'" method="post">'.
                                '<input type="hidden" name="id_quiz" value="'.$row['id'].'">'.
                                '<input type="hidden" name="id_user" value="'.$user_id.'">'.
                                '<button type="submit" class="btn m-btn m-btn--gradient-from-danger m-btn--gradient-to-warning">اشتراك</button>'.
                            '</form> </td>'.
