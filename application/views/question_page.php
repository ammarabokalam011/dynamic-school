							...
							<ul class="nav nav-tabs m-tabs m-tabs-line   m-tabs-line--left m-tabs-line--primary" role="tablist">
								<li class="nav-item m-tabs__item">
									<a class="nav-link m-tabs__link active" data-toggle="tab" href="#m_user_profile_tab_1" role="tab">
										...
										<?php echo '<h3>'.$question['text_questions'].'</h3>';?>
									</a>
								</li>
						...
						<div class="tab-pane active" id="m_user_profile_tab_1">
							...
							<table class="table table-striped- table-bordered table-hover table-checkable" id="m_table_1">
								...
								<tr>
									...
									<?php
									for ($i = 1; $i <= 4; $i++) {
										echo  '<th><form action="'.base_url('user_quiz/answer_answer').'" method="post">'.
											'<input type="hidden" name="id_question" value="'.$question['id'].'">'.
											'<input type="hidden" name="id_quiz" value="'.$id_quiz.'">'.
											'<input type="hidden" name="id_user" value="'.$id_user.'">'.
											'<input type="hidden" name="answer" value="'.$question['answer'.$i].'">'.
											'<button type="submit" >'.$question['answer'.$i].'</button></td>'.
											'</form></th>';
									} ?>
								</tr>
								</thead>
